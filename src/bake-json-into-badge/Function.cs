...
namespace bake_json_into_badge
{
    public class Function
    {
        ...
        private static void Main(string[] args)
        {
            ...
            //using (FileStream stream = new FileStream("./wpf.png", FileMode.Create))
            //{
            ...
            using (var image = Image.Load(imgBadge))
            {
                var meta = image.Metadata.GetPngMetadata();
                meta.TextData.Add(new PngTextData("openbadges", json, "", ""));

                using (var stream = new MemoryStream())
                {
                    image.SaveAsPng(stream);
                    image.SaveAsPng("./dnc.png");

                    badge_byte = stream.GetBuffer();
                }
            }
